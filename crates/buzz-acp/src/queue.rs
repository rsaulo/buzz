...
pub struct QueuedEvent {
    ...
    /// Whether somebody is owed an answer for this event. See
    /// [`crate::filter::MatchedRule::requires_response`].
...
impl QueuedEvent {
    ...
    pub fn session_key(&self) -> SessionKey {
        match event_thread_root(&self.event) {
...
/// One event handed to a live turn by a native steer, and its fate so far.
#[derive(Debug, Clone)]
struct PendingSteer {
    event: QueuedEvent,
    /// Turn generation that accepted the steer write. Settlement is scoped to
...
impl FlushBatch {
    ...
    /// Whether anyone in this batch is owed an answer.
    ///
    /// Counts `cancelled_events` too: a mention that was interrupted and merged
    /// into this re-prompt is still unanswered, and dropping it from the
    /// predicate would let a silent retry look like legitimate restraint.
...
///   in_flight_sessions:   HashSet<Uuid>
...
/// Transitions:
///   push(event):
///     if dedup_mode == Drop AND in_flight_sessions.contains(event.channel_id):
///       debug log + discard
...
pub struct EventQueue {
    queues: HashMap<SessionKey, VecDeque<QueuedEvent>>,
    in_flight_sessions: HashSet<SessionKey>,
    /// Per-conversation deadline for auto-expiring stuck in-flight entries.
    in_flight_deadlines: HashMap<SessionKey, Instant>,
    /// Number of events in each in-flight batch (for expiry logging).
    in_flight_batch_sizes: HashMap<SessionKey, usize>,
    /// Turn generation bound by the dispatcher after a batch is claimed.
    /// A late completion may clear state only when this ID still matches.
    in_flight_turn_ids: HashMap<SessionKey, String>,
    retry_after: HashMap<SessionKey, Instant>,
    /// Per-conversation retry attempt counter for backoff / dead-lettering.
    retry_counts: HashMap<SessionKey, u32>,
    dedup_mode: DedupMode,
    ...
    cancelled_batches: HashMap<SessionKey, Vec<BatchEvent>>,
    ...
    cancel_reasons: HashMap<SessionKey, CancelReason>,
...
impl EventQueue {
    ...
    pub fn push(&mut self, event: QueuedEvent) -> bool {
        ...
        if matches!(self.dedup_mode, DedupMode::Drop) && self.in_flight_sessions.contains(&key) {
            tracing::debug!(
                session = %key,
    ...
    /// inserts into `in_flight_sessions`, and returns the batch.
    pub fn flush_next(&mut self) -> Option<FlushBatch> {
        self.flush_next_excluding(&HashSet::new())
    }

    /// Flush the oldest eligible conversation not present in `excluded`.
    /// Dispatch uses this to skip a conversation whose owning agent is busy
    /// while still filling idle agents with unrelated work.
    pub fn flush_next_excluding(&mut self, excluded: &HashSet<SessionKey>) -> Option<FlushBatch> {
        let now = Instant::now();
        ...
        let expired: Vec<SessionKey> = self
            ...
            .collect();
        for key in expired {
            if let Some(turn_id) = self.in_flight_turn_ids.get(&key) {
                tracing::error!(
                    session = %key,
                    turn_id,
                    deadline_secs = self.in_flight_deadline.as_secs(),
                    "BUG: bound turn exceeded the queue backstop; preserving serialization"
                );
                self.in_flight_deadlines
                    .insert(key, now + self.in_flight_deadline);
                continue;
            }
            ...
            self.recover_withheld_for_expired_session(&key);
        }

        // Find the conversation whose head event has the oldest received_at,
        // excluding in-flight and throttled ones. Two threads of the same
        // channel are independent here — that is the point of this phase.
        ...
        let session_key = match session_key {
            ...
            None => {
                let cancelled_key = self
                    .cancelled_batches
                    .keys()
                    .find(|key| !self.in_flight_sessions.contains(key) && !excluded.contains(key))
                    .cloned();
                match cancelled_key {
                    Some(key) => {
                        ...
                        self.in_flight_sessions.insert(key.clone());
                        self.in_flight_deadlines
        ...
        //
        // No thread filtering is needed here: the queue is keyed by
        // conversation, so everything in it already belongs to one session.
        let queue = self.queues.entry(session_key.clone()).or_default();
        ...
        if self.queues.get(&session_key).is_some_and(|q| q.is_empty()) {
            self.queues.remove(&session_key);
        }

        self.in_flight_sessions.insert(session_key.clone());
        self.in_flight_deadlines
            .insert(session_key.clone(), now + self.in_flight_deadline);
        self.in_flight_batch_sizes
            .insert(session_key.clone(), events.len());
        ...
        let cancel_reason = if cancelled_events.is_empty() {
            self.cancel_reasons.remove(&session_key);
            None
        } else {
            self.cancel_reasons.remove(&session_key)
        };
    ...
    ///
    /// Removes the channel from `in_flight_sessions` and `in_flight_deadlines`.
    ///
    ...
    pub fn mark_complete(&mut self, key: &SessionKey) {
        self.in_flight_sessions.remove(key);
        self.in_flight_deadlines.remove(key);
    ...
    /// Release a batch that was flushed but never assigned to an agent.
    /// Unlike `mark_complete`, this must not reset retry/dead-letter history:
    /// no processing attempt occurred.
    pub fn release_unclaimed(&mut self, key: &SessionKey) {
        self.in_flight_sessions.remove(key);
        self.in_flight_deadlines.remove(key);
        self.in_flight_batch_sizes.remove(key);
        self.in_flight_turn_ids.remove(key);
    ...
    pub fn bind_in_flight_turn(&mut self, key: &SessionKey, turn_id: String) {
        debug_assert!(self.in_flight_sessions.contains(key));
        self.in_flight_turn_ids.insert(key.clone(), turn_id);
    }

    /// Complete a turn only if it still owns this conversation's in-flight slot.
    /// Returns `false` for a late result from an expired/replaced generation.
    pub fn mark_complete_turn(&mut self, key: &SessionKey, turn_id: &str) -> bool {
        if self.in_flight_turn_ids.get(key).map(String::as_str) != Some(turn_id) {
            tracing::warn!(
                session = %key,
                turn_id,
                current_turn_id = ?self.in_flight_turn_ids.get(key),
    ...
    pub fn requeue(&mut self, batch: FlushBatch) -> Option<FlushBatch> {
        ...
        //
        // BOTH buckets, not just `events`. A merged re-prompt carries the work
        // an interrupted turn was already handling in `cancelled_events`; a
        // mention that was interrupted and then died in the merged retry used to
        // vanish here with no error anywhere. Cancelled events are normalised
        // into ordinary queued events rather than returned to the cancelled side
        // table: that table is exempt from `retry_after` in the cancelled-only
        // flush fallback, so restoring there would let a throttled batch
        // re-flush immediately and spin — and the merge annotation frames
        // in-progress interrupted work, of which a failed turn has none.
        // Cancelled work first: it is the older half (the turn that was
        // interrupted), so FIFO puts it ahead of the events that arrived after.
        ...
        self.retry_after.insert(key.clone(), Instant::now() + delay);
        None
    }
    ...
    ///
    /// Does NOT set `retry_after`. Does NOT remove from `in_flight_sessions` —
    ...
    pub fn requeue_preserve_timestamps(&mut self, batch: FlushBatch) {
        ...
        // A cancelled-only fallback stores its events in `events`; a merged
        // regular batch stores prior interrupted work in `cancelled_events`.
        // Restore both shapes without losing their framing metadata.
        let cancelled_only = cancel_reason.is_some() && cancelled_events.is_empty();
        if cancelled_only {
            let entry = self.cancelled_batches.entry(key.clone()).or_default();
            let mut restored = events;
            restored.append(entry);
            *entry = restored;
            if let Some(reason) = cancel_reason {
                self.cancel_reasons.entry(key).or_insert(reason);
            }
            return;
        }
        if !cancelled_events.is_empty() {
            let entry = self.cancelled_batches.entry(key.clone()).or_default();
            let mut restored = cancelled_events;
            restored.append(entry);
            *entry = restored;
            if let Some(reason) = cancel_reason {
                self.cancel_reasons.entry(key.clone()).or_insert(reason);
            }
        }
    ...
    pub fn requeue_as_cancelled(&mut self, batch: FlushBatch, reason: CancelReason) {
        ...
        self.cancel_reasons.insert(key.clone(), reason);
    }
    ...
    pub fn has_flushable_work(&mut self) -> bool {
        ...
        self.queues.iter().any(|(key, q)| {
            ...
            .cancelled_batches
            .keys()
            .any(|key| !self.in_flight_sessions.contains(key))
    }
    ...
    /// that method excludes `retry_after`-throttled conversations because they
    /// are not flushable *right now*, but the events are still queued and MUST
    /// be delivered once the backoff deadline passes. Idle-pool-sleep teardown
    ...
    ///
    /// Keyed by [`SessionKey`], not channel: a sibling thread's queued work in
    /// the same channel is its own conversation, and upstream's channel-keyed
    /// form would let one thread's in-flight turn mask another thread's pending
    /// batch — which is exactly the stranding this gate exists to prevent.
    pub fn has_undispatched_work(&self) -> bool {
        let has_queued = self
            .queues
            .iter()
            .any(|(key, q)| !q.is_empty() && !self.in_flight_sessions.contains(key));
        ...
        let has_withheld = self
            ...
            .iter()
            .any(|(key, v)| !v.is_empty() && !self.in_flight_sessions.contains(key));
    ...
    #[cfg(test)]
    pub fn queued_event_count(&self, key: &SessionKey) -> usize {
    ...
    #[cfg(test)]
    pub fn set_retry_count_for_test(&mut self, key: &SessionKey, count: u32) {
        self.retry_counts.insert(key.clone(), count);
    }
    ...
    pub fn drain_channel(&mut self, channel_id: Uuid) -> Vec<String> {
        // Channel-level on purpose: losing access to a channel invalidates
        // every conversation inside it, ambient and threads alike.
        ...
        for key in keys {
            ...
            self.withheld_native_steer.remove(&key);
        }
        // Preserve in_flight_sessions AND in_flight_deadlines: the in-flight
        ...
        // removing in_flight_sessions would disable auto-expiry and leave a
        // wedged task permanently blocking the channel.
        ids
    }

    /// Whether a prompt is currently in-flight for any conversation in the
    /// given channel.
    pub fn is_channel_in_flight(&self, channel_id: Uuid) -> bool {
        self.in_flight_sessions
            .iter()
    ...
    pub fn has_in_flight(&self) -> bool {
        !self.in_flight_sessions.is_empty()
    }

    // ── Native steer side table ───────────────────────────────────────────
    //
    // While a native steer write is in flight for a specific queued event, that
    // event is moved out of `queues` into `withheld_native_steer` so
    // `flush_next` / `has_flushable_work` / the contiguous drain cannot see it —
    // closing the race between `mark_complete` (which clears
    // `in_flight_sessions`) and the ack arriving on the main loop.
    //
    // A `Success` ack does NOT drop the event. Delivery is not an answer: the
    // agent has the message, but the turn may still end without responding to
    // it. The ack instead stamps the entry with the turn's output epoch at
    // acceptance, and the turn's terminal settlement decides:
    //
    //   output after the stamp        → answered, retire
    //   no output after, and owed     → unanswered, release for a normal retry
    //   no output after, and passive  → silence was correct, retire
    //   ack never arrived             → delivery unknown, release
    //
    // Both states live in ONE table on purpose. Upstream keeps two and has to
    // remember to settle both at every terminal boundary; a single table with an
    // optional epoch makes "settled half the state" unrepresentable.

    /// Move a queued event out of `queues[key]` into the side table to withhold
    /// it from `flush_next` while a native steer is in flight.
    ///
    /// Returns `true` if the event was found and withheld, `false` if the event
    /// id was not present (race-safe no-op: it may already have been drained,
    /// removed, or never queued).
    ///
    /// Must be called synchronously from the mode-gate fork immediately after
    /// `pool.send_steer` returns, and before any watcher task is spawned, so the
    /// withhold is established before `mark_complete` / any subsequent
    /// `flush_next` tick can run.
    ...
    /// Record that a native steer was delivered into `turn_id`, stamping the
    /// turn's output epoch at the moment of acceptance.
    ///
    /// Returns `false` when the event is no longer pending for that exact turn —
    /// the turn already settled and released it, or the ack belongs to a turn
    /// that is gone. That answer is what makes a late ack a total no-op: no
    /// second copy, no deadline extension for an unrelated turn, and no
    /// cancel+merge fallback aimed at a turn that already ended.
    pub fn record_delivered_steer(
        ...
        let Some(entry) = entries
            .iter_mut()
            .find(|p| p.event.event.id.to_hex() == event_id && p.turn_id == turn_id)
        else {
            return false;
        };
        entry.delivered_at_epoch = Some(accepted_at_epoch);
        true
    }

    /// Release a single withheld event back to the front of `queues[key]`,
    /// preserving its original `received_at`.
    ///
    /// Called for the failure-shaped acks, where the steer did not land.
    /// Returns `false` if the event was already settled or never withheld — the
    /// caller uses that to suppress a fallback signal aimed at nothing.
    pub fn release_native_steer(&mut self, key: &SessionKey, event_id: &str) -> bool {
        let Some(entries) = self.withheld_native_steer.get_mut(key) else {
            return false;
        };
        let Some(pos) = entries
            .iter()
            .position(|p| p.event.event.id.to_hex() == event_id)
        else {
            return false;
        };
        ...
        self.push_front_with_cap(key, entry.event, "release_native_steer");
        true
    }
    ...
    pub fn settle_turn_steers(&mut self, key: &SessionKey, turn_id: &str) -> SteerSettlement {
        ...
        // Reverse so the per-entry push_front composes back to arrival order.
    ...
    /// its state.
    ///
    /// Called from the `in_flight_deadline` expiry blocks: if a turn never
    /// reports a result (read loop hung, watcher never posted) nothing will ever
    /// settle its ledger, and the events would be orphaned. Recover, do not
    /// log-and-drop — at worst the agent sees a message twice, which is the
    /// duplicate-over-silent-loss trade this whole path is built on.
    fn recover_withheld_for_expired_session(&mut self, key: &SessionKey) {
        let Some(entries) = self.withheld_native_steer.remove(key) else {
            return;
        };
        let n = entries.len();
        for entry in entries.into_iter().rev() {
            self.push_front_with_cap(key, entry.event, "withheld-steer recovery");
        }
        tracing::warn!(
            session = %key,
            recovered = n,
            "in-flight expiry recovered steer event(s) — \
             the turn never settled them; normal dispatch will deliver"
        );
    }

    /// Push a recovered event back to the head of its conversation, preserving
    /// `received_at`, and enforce the per-conversation depth cap.
    fn push_front_with_cap(&mut self, key: &SessionKey, event: QueuedEvent, whence: &str) {
        let queue = self.queues.entry(key.clone()).or_default();
        ...
        while queue.len() > MAX_PENDING_PER_CHANNEL {
            ...
            tracing::warn!(
                ...
                limit = MAX_PENDING_PER_CHANNEL,
                whence,
                "steer release overflow — dropped newest event to enforce cap"
            );
        }
    }
    ...
    pub fn compact_expired_state(&mut self) {
        ...
        self.retry_counts.retain(|ch, _| {
            self.retry_after.contains_key(ch)
                ...
                || self.in_flight_sessions.contains(ch)
        });
    }
}
...
/// The prompt sections that do not change for the life of a session: workspace,
/// base prompt, persona, team instructions, core memory, and channel canvas.
///
    ...
    pub(crate) fn sections(&self) -> Vec<String> {
        ...
        if let Some(bp) = self.base_prompt {
            if let Some(workspace) = self
                .cwd
                .and_then(|cwd| workspace_section(cwd, self.workspace_kind))
            {
...
///    `[Workspace]`, `[Agent Memory — core]`, `[Channel Canvas]`. Legacy agents
///    only, and only on the session's first message (see `standing_context_sent`)
...
pub fn format_prompt(batch: &FlushBatch, args: &FormatPromptArgs<'_>) -> Vec<String> {
    ...
    // workspace, and canvas. Modern agents received all of it via the system role in
...
mod tests {
    ...
    fn test_mark_complete_enables_flush() {
        ...
        q.mark_complete(&SessionKey::ambient(ch));
        assert!(!any_in_flight(&q));
    ...
    #[test]
    fn flush_excluding_busy_affinity_skips_only_that_conversation() {
        let mut q = EventQueue::new(DedupMode::Queue);
        let blocked = Uuid::new_v4();
        let available = Uuid::new_v4();
        let blocked_key = SessionKey::ambient(blocked);
        ...
        let batch = q
            .flush_next_excluding(&HashSet::from([blocked_key.clone()]))
            .expect("unrelated conversation remains dispatchable");
        assert_eq!(batch.channel_id, available);
        assert_eq!(q.queued_event_count(&blocked_key), 1);
    }
    ...
    fn test_multi_channel_interleave() {
        ...
        q.mark_complete(&SessionKey::ambient(ch_a));
    ...
    fn test_requeue_preserves_events() {
        ...
        queue.requeue(batch);
        queue.mark_complete(&SessionKey::ambient(ch));

        // retry_after is set, so manually clear it for this test.
        queue.retry_after.remove(&SessionKey::ambient(ch));
    ...
    fn test_has_undispatched_work_false_when_truly_empty_or_in_flight() {
        ...
        queue.mark_complete(&batch.session_key());
        assert!(!queue.has_undispatched_work());
        assert!(!queue.has_in_flight());
    }

    /// The sleep gate is keyed by conversation, not channel. A turn in flight
    /// for one thread must not hide a sibling thread's queued batch: upstream's
    /// channel-keyed form would report "nothing undispatched" here and let the
    /// pool tear down with real work waiting in the same channel.
    #[test]
    fn test_undispatched_work_is_visible_across_sibling_threads() {
        let mut queue = EventQueue::new(DedupMode::Queue);
        let ch = Uuid::new_v4();
        let root_a = "a".repeat(64);
        let root_b = "b".repeat(64);

        queue.push(make_queued_in_thread(ch, "thread A", &root_a));
        let batch = queue.flush_next().expect("thread A dispatches");
        assert_eq!(batch.root.as_deref(), Some(root_a.as_str()));
        assert!(queue.has_in_flight());
        assert!(
            !queue.has_undispatched_work(),
            "only thread A existed, and it is in flight"
        );

        // A sibling thread in the SAME channel queues work while A runs.
        queue.push(make_queued_in_thread(ch, "thread B", &root_b));
        assert!(
            queue.has_undispatched_work(),
            "thread B's batch is undispatched even though thread A is in flight"
        );
    }
    ...
    fn test_requeue_interleaves_with_other_channels() {
        ...
        queue.mark_complete(&SessionKey::ambient(ch_a));
    ...
    fn test_multiple_channels_in_flight_simultaneously() {
        ...
        // Both in-flight.
        assert_eq!(q.in_flight_sessions.len(), 2);
        ...
        // Complete B.
        q.mark_complete(&SessionKey::ambient(ch_b));
        assert!(!any_in_flight(&q));
    }
    ...
    fn test_flush_next_picks_oldest_non_throttled() {
        ...
        q.mark_complete(&SessionKey::ambient(ch_a));
        q.mark_complete(&SessionKey::ambient(ch_b));
        q.mark_complete(&SessionKey::ambient(ch_c));
    }
    ...
    fn test_mark_complete_clears_only_specified_channel() {
        ...
        // Complete only A.
        q.mark_complete(&SessionKey::ambient(ch_a));
        assert_eq!(q.in_flight_sessions.len(), 1);
        assert!(q.in_flight_sessions.contains(&SessionKey::ambient(ch_b)));
        assert!(!q.in_flight_sessions.contains(&SessionKey::ambient(ch_a)));
    ...
    fn test_requeue_preserve_timestamps() {
        ...
        q.release_unclaimed(&SessionKey::ambient(ch));
    ...
    #[test]
    fn release_unclaimed_preserves_retry_counter() {
        let mut q = EventQueue::new(DedupMode::Queue);
        let ch = Uuid::new_v4();
        let key = SessionKey::ambient(ch);
        q.push(make_queued(ch, "retrying"));
        q.retry_counts.insert(key.clone(), 4);

        let batch = q.flush_next().expect("batch");
        q.requeue_preserve_timestamps(batch);
        q.release_unclaimed(&key);

        assert_eq!(q.retry_counts.get(&key), Some(&4));
    }
    ...
    fn test_requeue_dead_letters_after_max_retries() {
        ...
        assert!(!q.retry_counts.contains_key(&SessionKey::ambient(ch)));
        assert!(!q.retry_after.contains_key(&SessionKey::ambient(ch)));
    }
    ...
    fn test_retry_throttle_blocks_requeue_channel() {
        ...
        q.mark_complete(&SessionKey::ambient(ch2));
        let batch3 = q
            .flush_next()
    ...
    fn test_flush_splits_one_channel_into_one_batch_per_thread() {
        ...
        // No mark_complete between these: threads of one channel are
        // independently dispatchable, so B and ambient must flush while A is
        // still in flight.
        let second = q.flush_next().expect("thread B flushes while A is running");
        ...
        let third = q.flush_next().expect("ambient message should flush too");
        ...
        q.mark_complete(&first.session_key());
        q.mark_complete(&second.session_key());
        q.mark_complete(&third.session_key());
        assert!(!q.has_in_flight());
    }

    /// The concurrency claim of this phase, stated as a test: turns in
    /// different threads of one channel run side by side, while turns in the
    /// same thread still queue behind each other.
    #[test]
    fn test_threads_of_one_channel_dispatch_concurrently_but_a_thread_serializes() {
        let ch = Uuid::new_v4();
        let root_a = root_id('a');
        let root_b = root_id('b');
        let mut q = EventQueue::new(DedupMode::Queue);
        ...
        let a = q.flush_next().expect("thread A dispatches");
        let b = q
            .flush_next()
            .expect("thread B dispatches without waiting for A");
        assert_eq!(a.session_key(), SessionKey::thread(ch, &root_a));
        assert_eq!(b.session_key(), SessionKey::thread(ch, &root_b));
        assert!(q.is_session_in_flight(&a.session_key()));
        assert!(q.is_session_in_flight(&b.session_key()));
        assert!(q.is_channel_in_flight(ch));

        // A second message in thread A must NOT dispatch while A is running.
        assert!(q.push(make_queued_in_thread(ch, "a2", &root_a)));
        assert!(
            q.flush_next().is_none(),
            "the same thread still serializes behind its own in-flight turn"
        );

        // Once A completes, its backlog flows — and only its backlog.
        q.mark_complete(&a.session_key());
        let a2 = q.flush_next().expect("thread A resumes after completing");
        assert_eq!(a2.session_key(), SessionKey::thread(ch, &root_a));
        assert_eq!(a2.events[0].event.content, "a2");
    }
    ...
    fn test_compact_cleans_orphaned_retry_counts() {
        ...
        q.retry_counts.insert(SessionKey::ambient(ch), 3);
        q.compact_expired_state();
        assert!(
            !q.retry_counts.contains_key(&SessionKey::ambient(ch)),
    ...
    fn test_compact_preserves_retry_counts_when_in_flight() {
        ...
        assert!(q.in_flight_sessions.contains(&SessionKey::ambient(ch)));
        assert!(q
            .queues
            .get(&SessionKey::ambient(ch))
    ...
    fn test_compact_preserves_retry_counts_with_queued_events() {
        ...
        q.push(make_queued(ch, "msg1"));
        q.retry_counts.insert(SessionKey::ambient(ch), 2);
    ...
    fn test_native_steer_withhold_only_channel_not_flushable() {
        ...
        assert!(q.mark_native_steer_pending(&SessionKey::ambient(ch), &event_id, "test-turn"));
    ...
    fn test_native_steer_earlier_events_flush_during_ack_window() {
        ...
        assert!(q.mark_native_steer_pending(&SessionKey::ambient(ch), &e3_id, "test-turn"));
        ...
        q.release_native_steer(&SessionKey::ambient(ch), &e3_id);
    ...
    fn test_native_steer_expiry_recovers_withheld() {
        ...
        q.in_flight_sessions.insert(SessionKey::ambient(ch));
        q.in_flight_deadlines
            .insert(SessionKey::ambient(ch), Instant::now());
        q.in_flight_batch_sizes.insert(SessionKey::ambient(ch), 1);
        assert!(q.mark_native_steer_pending(&SessionKey::ambient(ch), &event_id, "test-turn"));
    ...
    fn test_native_steer_bulk_release_preserves_fifo() {
        ...
        // queue front.
        assert!(q.mark_native_steer_pending(&SessionKey::ambient(ch), &e1_id, "test-turn"));
        assert!(q.mark_native_steer_pending(&SessionKey::ambient(ch), &e2_id, "test-turn"));
        assert!(q.mark_native_steer_pending(&SessionKey::ambient(ch), &e3_id, "test-turn"));
        assert_eq!(pending_count(&q), 0);
        assert_eq!(
            q.withheld_native_steer
        ...
        q.in_flight_batch_sizes.insert(SessionKey::ambient(ch), 3);
        assert!(q.has_flushable_work());
    ...
    // ── Delivered-steer settlement ──────────────────────────────────────────
    //
    // A `Success` ack proves the agent HAS the message, not that it answered.
    // These pin the four verdicts, plus the orderings where the ack and the
    // turn's own result race each other.
    ...
    /// The incident, exactly: the steer was accepted mid-turn and the turn then
    /// ended having produced nothing. Before the ledger, the ack dropped the
    /// event outright and the mention was gone for good.
    #[test]
    fn steered_mention_survives_a_turn_that_produced_nothing() {
        let mut q = EventQueue::new(DedupMode::Queue);
        let ch = Uuid::new_v4();
        let key = SessionKey::ambient(ch);
    ...
    /// Output that happened BEFORE the steer landed does not answer it. This is
    /// the case a per-turn boolean cannot express, and the reason the ledger
    /// stores an epoch instead of a flag.
    #[test]
    fn steered_mention_is_requeued_when_output_only_preceded_it() {
        let mut q = EventQueue::new(DedupMode::Queue);
        let ch = Uuid::new_v4();
        let key = SessionKey::ambient(ch);
    ...
    /// silence-as-success contract: nobody asked the agent anything.
    #[test]
    fn passive_steered_event_is_not_requeued_when_unanswered() {
        let mut q = EventQueue::new(DedupMode::Queue);
        let ch = Uuid::new_v4();
        let key = SessionKey::ambient(ch);
    ...
    /// The ack lost the race with its own turn's result. Delivery is unknown,
    /// so the event is released regardless of obligation rather than guessed at.
    #[test]
    fn steer_whose_ack_never_arrived_is_released_at_settlement() {
        let mut q = EventQueue::new(DedupMode::Queue);
        let ch = Uuid::new_v4();
        let key = SessionKey::ambient(ch);
    ...
    /// The exact ordering regression: the turn settles first, then the late
    /// `Success` runs. It must find nothing and queue no second copy.
    #[test]
    fn late_success_ack_after_settlement_is_a_no_op() {
        let mut q = EventQueue::new(DedupMode::Queue);
        let ch = Uuid::new_v4();
        let key = SessionKey::ambient(ch);
        let event_id = steer(&mut q, &key, make_queued(ch, "mid-turn mention"), false);
        ...
        assert!(
            !q.release_native_steer(&key, &event_id),
            "and a late failure-shaped ack must report the same"
        );
        assert_eq!(pending_count(&q), 1, "released exactly once");
        assert!(q.withheld_native_steer.is_empty());
    }

    /// Settlement is scoped to the exact turn. A replacement turn must not
    /// judge — or release — the steer an earlier turn accepted.
    #[test]
    fn settlement_ignores_steers_belonging_to_another_turn() {
        let mut q = EventQueue::new(DedupMode::Queue);
        let ch = Uuid::new_v4();
        let key = SessionKey::ambient(ch);
        steer(&mut q, &key, make_queued(ch, "mid-turn mention"), true);
    ...
    /// Two threads of one channel are separate conversations: settling one must
    /// leave the other's steer untouched.
    #[test]
    fn settlement_does_not_cross_sibling_threads() {
        let mut q = EventQueue::new(DedupMode::Queue);
        let ch = Uuid::new_v4();
        let root_a = "a".repeat(64);
        let root_b = "b".repeat(64);
        let key_a = SessionKey::thread(ch, root_a.clone());
        let key_b = SessionKey::thread(ch, root_b.clone());
        steer(
            &mut q,
            &key_a,
    ...
    /// it a delivered event would be orphaned in the ledger forever.
    #[test]
    fn expiry_releases_delivered_steer_as_unanswered() {
        let mut q = EventQueue::new(DedupMode::Queue);
        let ch = Uuid::new_v4();
        let key = SessionKey::ambient(ch);
        ...
        assert!(q.has_flushable_work(), "expiry must recover the event");
        let batch = q.flush_next().expect("recovered");
        assert_eq!(batch.events[0].event.id.to_hex(), event_id);
    }

    /// A merged re-prompt carries interrupted work in `cancelled_events`. Both
    /// buckets must survive a retry: a mention that was interrupted and then
    /// died in the merged retry used to vanish with no error anywhere.
    #[test]
    fn requeue_preserves_both_buckets_of_a_merged_batch() {
        let mut q = EventQueue::new(DedupMode::Queue);
        let ch = Uuid::new_v4();
        let key = SessionKey::ambient(ch);

        // First turn is interrupted, so its events become cancelled carryover.
        q.push(make_queued(ch, "interrupted mention"));
        let first = q.flush_next().expect("first batch");
        q.requeue_as_cancelled(first, CancelReason::Steer);
        q.mark_complete(&key);

        // A new event arrives and merges with the carryover.
        q.push(make_queued(ch, "new mention"));
        let merged = q.flush_next().expect("merged batch");
        assert_eq!(merged.events.len(), 1);
        assert_eq!(merged.cancelled_events.len(), 1);

        // The merged turn now fails and is retried.
        assert!(q.requeue(merged).is_none(), "requeued, not dead-lettered");
        q.mark_complete(&key);

        assert_eq!(
            pending_count(&q),
            2,
            "both the interrupted mention and the new one survive the retry"
        );
        let contents: Vec<String> = q
            .queues
            .get(&key)
            .expect("queue")
            .iter()
            .map(|qe| qe.event.content.clone())
            .collect();
        assert_eq!(
            contents,
            vec!["interrupted mention".to_string(), "new mention".to_string()],
            "cancelled work is older, so it stays ahead in FIFO"
        );
    }
    ...
    fn extend_in_flight_deadline_advances_existing_entry() {
        ...
        q.in_flight_sessions.insert(SessionKey::ambient(ch));
        q.in_flight_deadlines
            .insert(SessionKey::ambient(ch), old_deadline);

        q.extend_in_flight_deadline(&SessionKey::ambient(ch), 7200);
        let new = *q.in_flight_deadlines.get(&SessionKey::ambient(ch)).unwrap();
        assert!(
            new > old_deadline,
    ...
    fn extend_in_flight_deadline_is_monotonic() {
        ...
        q.in_flight_sessions.insert(SessionKey::ambient(ch));
        q.in_flight_deadlines
            .insert(SessionKey::ambient(ch), far_future);

        q.extend_in_flight_deadline(&SessionKey::ambient(ch), 7200);
        let after = *q.in_flight_deadlines.get(&SessionKey::ambient(ch)).unwrap();
    ...
    fn extend_in_flight_deadline_noop_after_mark_complete() {
        ...
        q.mark_complete(&SessionKey::ambient(ch));
        assert!(!q.in_flight_deadlines.contains_key(&SessionKey::ambient(ch)));

        q.extend_in_flight_deadline(&SessionKey::ambient(ch), 7200);
        assert!(
            !q.in_flight_deadlines.contains_key(&SessionKey::ambient(ch)),
            "extend after mark_complete must not resurrect a deadline"
        );
    }

    #[test]
    fn stale_completion_cannot_clear_a_replacement_turn() {
        let mut q = EventQueue::new(DedupMode::Queue);
        let key = SessionKey::ambient(Uuid::new_v4());
        q.in_flight_sessions.insert(key.clone());
        q.bind_in_flight_turn(&key, "turn-a".into());

        // Simulate the backstop expiring A and dispatching replacement B.
        q.mark_complete(&key);
        q.in_flight_sessions.insert(key.clone());
        q.bind_in_flight_turn(&key, "turn-b".into());

        assert!(!q.mark_complete_turn(&key, "turn-a"));
        assert!(q.in_flight_sessions.contains(&key));
        assert_eq!(
            q.in_flight_turn_ids.get(&key).map(String::as_str),
            Some("turn-b")
        );

        assert!(q.mark_complete_turn(&key, "turn-b"));
        assert!(!q.in_flight_sessions.contains(&key));
    }
    ...
    fn compact_expired_state_preserves_extended_in_flight_deadline() {
        ...
        q.in_flight_sessions.insert(SessionKey::ambient(ch));
        q.in_flight_deadlines
            .insert(SessionKey::ambient(ch), extended);
        ...
        assert_eq!(
            *q.in_flight_deadlines.get(&SessionKey::ambient(ch)).unwrap(),
            extended,
    ...
    #[test]
    fn expired_bound_turn_preserves_same_conversation_serialization() {
        let mut q = EventQueue::new(DedupMode::Queue);
        let ch = Uuid::new_v4();
        let key = SessionKey::ambient(ch);
        q.in_flight_sessions.insert(key.clone());
        q.in_flight_deadlines.insert(key.clone(), Instant::now());
        q.in_flight_batch_sizes.insert(key.clone(), 1);
        q.bind_in_flight_turn(&key, "still-running".into());
        q.push(make_queued(ch, "must-wait"));
    ...
    fn extended_deadline_prevents_flush_next_expiry() {
        ...
        assert!(
            q.in_flight_sessions.contains(&SessionKey::ambient(ch)),
    ...
    fn repeated_extend_in_flight_deadline_is_monotonic() {
        ...
        q.extend_in_flight_deadline(&SessionKey::ambient(ch), 7200);
        let after_first = *q.in_flight_deadlines.get(&SessionKey::ambient(ch)).unwrap();

        q.extend_in_flight_deadline(&SessionKey::ambient(ch), 7200);
        let after_second = *q.in_flight_deadlines.get(&SessionKey::ambient(ch)).unwrap();
    ...
    fn description_batch(ch: Uuid, event: Event) -> FlushBatch {
        // Derive the batch's conversation from the event itself, the same rule
        // `QueuedEvent::session_key` uses — so a threaded event produces a
        // thread-scope batch and a top-level one stays ambient.
        let root = event_thread_root(&event);
        FlushBatch {
            channel_id: ch,
            root,
            events: vec![BatchEvent {
