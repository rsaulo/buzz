...
/// rebuild per channel per [`NEGATIVE_CACHE_WINDOW`]. Positive hits are rebuilt
/// before use so a new session observes removed declarations, conflicts, and
/// repositories replaced by escaping symlinks.
...
impl WorkspaceIndex {
    ...
    pub fn lookup(&self, channel_id: Uuid) -> Option<PathBuf> {
        ...
        if let Some(path) = state.channels.get(&channel_id).cloned() {
            if (self.is_directory)(&path) {
                tracing::debug!(
                    channel = %channel_id,
                    repository = %path.display(),
                    "workspace index revalidating cached repository"
                );
                Self::rebuild_locked(&mut state, "revalidated cached repository");
                if let Some(revalidated) = state.channels.get(&channel_id) {
                    if (self.is_directory)(revalidated) {
                        return Some(revalidated.clone());
                    }
                }
                state.negative_cache.insert(channel_id, now);
                return None;
            }
            tracing::warn!(
                channel = %channel_id,
...
mod tests {
    ...
    fn cache_miss_performs_exactly_one_rebuild() {
        ...
        assert_eq!(
            ...
            3,
            "positive hits are revalidated before a new session uses them"
        );
    }

    #[cfg(unix)]
    #[test]
    fn cached_repository_replaced_by_escaping_symlink_is_refused() {
        use std::os::unix::fs::symlink;
        ...
        let index = WorkspaceIndex::with_rebuilder(move || discover(&[&root_path]));
        let channel = Uuid::parse_str(CHANNEL_A).unwrap();

        std::fs::remove_dir_all(&repo).expect("remove cached repository");
        symlink(&outside_repo, &repo).expect("replace repository with symlink");
